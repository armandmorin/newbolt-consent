/*
  # Initial Schema Setup for ConsentHub

  1. New Tables
    - `users` - Stores user accounts
    - `clients` - Stores client websites
    - `branding_settings` - Stores branding customization
    - `subscriptions` - Stores subscription information
    - `plans` - Stores available subscription plans
    - `consent_logs` - Stores consent interaction logs
    - `activity_logs` - Stores user activity for auditing
...
CREATE POLICY "Admins can read their own clients"
  ON clients
  FOR SELECT
  TO authenticated
  USING (
...
CREATE POLICY "SuperAdmins can read all branding settings"
  ON branding_settings
  FOR SELECT
  TO authenticated
  USING (
...
CREATE POLICY "Admins can read their clients' consent logs"
  ON consent_logs
  FOR SELECT
  TO authenticated
  USING (
    EXISTS (
      ...
      WHERE clients.id = consent_logs.client_id
      AND (
        clients.admin_id = auth.uid() OR
        EXISTS (
          SELECT 1 FROM users
          WHERE users.id = auth.uid() AND users.role = 'superadmin'
        )
...
CREATE POLICY "SuperAdmins can read all activity logs"
  ON activity_logs
  FOR SELECT
  TO authenticated
  USING (
